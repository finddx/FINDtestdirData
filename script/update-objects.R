...
raw_instruments <- raw_instruments |>
  ...
  filter(purpose_of_submission =="Test Directory") |>
    mutate_all(as.character) |>
    mutate(across(-c(instrument_id, directory, test_to_be_listed_on_finds_web_page, find_website_area, disease_target, lab_vs_poc, type_of_technology, stage_of_development, test_format, region, confidentiality_level), ~ ifelse(confidentiality_level  %in% c("Level 2", "Level 3"), "Private Information", .))) |>
  mutate(lat=ifelse(lat=="Private Information", NA, lat),
         lng=ifelse(lng=="Private Information", NA, lng))
