...
raw_assays <- raw_assays |>
  ...
  mutate(across(-c(assay_id, directory, test_to_be_listed_on_finds_web_page, data_to_be_published_on_finds_web_page, find_website_area, disease_target, lab_vs_poc, type_of_technology, stage_of_development, region, confidentiality_level), ~ ifelse(confidentiality_level  %in% c("Level 2", "Level 3"), "Private Information", .))) |>
